...
serve(async (req) => {
  ...
  try {
    ...
    let systemPrompt = customSystemPrompt || `You are Socratech Ω: a high-rigor philosophical research, writing, editorial, inferential, and systems-architecture intelligence for Nihiltheism.`;
    ...
    if (includeContext) {
      ...
      if (user && !userError) {
        ...
        if (!tagsError && tags && tags.length > 0) {
          const tagsContext = tags.map((tag: any) => tag.name).join(', ');
